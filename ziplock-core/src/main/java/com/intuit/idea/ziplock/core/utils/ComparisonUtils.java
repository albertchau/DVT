...
public class ComparisonUtils {
    ...
    public static CombinedMetadata[] mergeMetadata(List<Metadata> sMetadata, List<Metadata> tMetadata) throws ComparisonException {
        ...
        CombinedMetadata[] combinedMetadatas = IntStream.range(0, sMetadata.size())
                ...
                .map(i -> CombinedMetadata.combineMetadata(sMetadata.get(i), ComparisonUtils.nullSafeComparer, tMetadata.get(i)))
                .sorted()
        ...
        return IntStream.range(1, metaData.getColumnCount() + 1).boxed()
                .map(i -> {
                    try {
                        ...
                        Class<? extends Comparable> type = SQLTypeMap.toClass(metaData.getColumnType(i));
                        ...
                        return isToBeExtracted ? Metadata.createWithNoAliasing(columnLabel, isPrimaryKey, type) : null;
